...
## 2026-04-29 — Фикс отмены автосписания для `web_*` пользователей

### Проблема

Пользователи жаловались, что кнопка отмены автосписания в настройках выглядит нерабочей и автосписание не отключается.

Корень проблемы был на фронте, а не в YooKassa и не в cron:
- `src/SettingsPage.js` блокировал `handleCancel()`, если у профиля отсутствовал `contact_id`
- backend `api/cancel-subscription.js` при этом использует только `telegram_id`
- для части веб-пользователей с ID вида `web_*` `contact_id` пустой, поэтому нажатие на кнопку не приводило ни к какому запросу

### Что исправлено

- В `src/SettingsPage.js` проверка перед отменой изменена с `!profile?.contact_id` на `!telegramId`
- Текст успешной отмены уточнён: отменяется именно автосписание, а подписка остаётся активной до конца оплаченного срока

### Важное правило

- Для автосписаний и отмены автосписаний `telegram_id` до сих пор является рабочим строковым ключом пользователя в БД
- Значения вида `web_*` валидны для `create-payment`, `yookassa-webhook`, `cron-subscriptions` и `cancel-subscription`
- На этих путях нельзя завязываться на `contact_id` и нельзя требовать числовой Telegram ID
- Ограничение касается только Telegram-уведомлений: для `web_*` пользователей сообщения бота могут не доставляться, но само списание и отмена работают

### Коммит и деплой

- Коммит: `8614497` — `fix(settings): allow auto-renew cancellation without contact id`
- Деплой: `git pull && docker compose up -d --build && docker compose restart app`
