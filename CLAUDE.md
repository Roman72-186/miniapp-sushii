...
**Product catalogs**: JSON files in `public/` → built into `build/`. Admin edits saved to `data/products/` which **take priority over `build/`** when served by Express. This means price changes in `public/` won't take effect if stale overrides exist in `data/products/`.
...
Everywhere: strip to digits, replace leading `8` with `7`, pad 10-digit to `7XXXXXXXXXX`. **No plus sign** except in YooKassa receipt (API requirement: `+7XXXXXXXXXX`). Function `normalizePhone()` exists in 4 frontend files + backend.
...
3. `UserContext` exposes: `telegramId`, `tarif`, `phone`, `profile`, `sync()`
4. `telegramId` sourced from Telegram WebApp SDK or `?telegram_id=` URL param
...
Generated by `buildWindows(start, end, windowDays)`. Tariff 490: every 15 days. Tariff 1190: every 30 days. Admin grants bypass tariff check. Stored in `data/gifts/{telegram_id}.json`.
...
При компактизации сохра‐
няй полный список изменённых файлов и текущий статус тестов.