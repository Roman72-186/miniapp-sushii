...
На Windows вывод `Get-Content`, `Select-String`, `git diff` или тестов в терминале может отображать UTF-8 кириллицу как mojibake (`Рџ...`), хотя сам файл сохранён корректно. Если патч или поиск не матчится на строках с кириллицей, не переписывать файл и не копировать искажённый текст из вывода терминала.

Правильный порядок:
- для `apply_patch` использовать ASCII-якоря вокруг нужного места: имена переменных, ключи JSON, route/path, function names;
- если нужен точный русский текст, сначала проверить файл через UTF-8-aware просмотр (`Select-String` обычно показывает корректнее, чем построчный `Get-Content` в текущей консоли);
- не менять кодировку файла и не делать массовую перекодировку ради одной правки;
- в отчётах ссылаться на это как на **Правило кириллического контекста**.
...
> ⚠️ **Правило `shop-v2.css`**: это override-слой (~960+ строк) для **Linear V3**-дизайна всех страниц магазина. Импортируется в `App.js` **последним** и перекрывает `shop.css`. Любые изменения дизайна магазина — **только** в `shop-v2.css`. Не трогать JSX и `shop.css` для косметики.

**Linear V3 design tokens** (определены в `shop-v2.css`, используются везде):
```css
--v3-bg:      #0F0F11   /* страница */
--v3-surface: #18181B   /* карточки */
--v3-raise:   #1F1F23   /* кнопки/raised элементы */
--v3-inset:   #141416   /* вдавленные поля */
--v3-border:  rgba(255,255,255,0.07)  /* линии */
--v3-accent:  #3CC8A1   /* акцент (teal) */
...
**Schema tables** (одинаковые для SQLite и PostgreSQL): `users`, `payments`, `transactions`, `referral_bonuses`, `gift_history`, `orders`, `email_notifications_log`. Runtime-миграции идут через `ALTER TABLE ... ADD COLUMN` (в SQLite — try/catch, в PG — `IF NOT EXISTS`). Колонки добавленные после initial schema:
- `users`: `partner_code`, `notes`, `last_address`, `last_pickup_point`, `first_name`, `last_name`, `middle_name`, `game_wins_today`, `game_day`, `game_current_word`, `game_word_status`, `game_session_id`, `email`
- `gift_history`: `address`, `gift_name`
- `orders`: `promo_code`, `has_promo_gift`, `has_threshold_gift` — логирование промокода и типов подарков
- `email_notifications_log (id, telegram_id, event_type, sent_at)` — дедупликация email-уведомлений (Resend)
...
**`WebRegistrationPrompt`** — глобальный overlay (поверх любой страницы), показывается когда `profile.needsWebRegistration === true` и не был отклонён в текущей сессии (`sessionStorage 'web_reg_dismissed'`). Предлагает legacy-пользователям (без JWT) зарегистрироваться на веб-сайте.
...
⚠️ Важная оговорка по подпискам: для платёжных и рекуррентных flow legacy-поле `telegram_id` всё ещё используется как **строковый ключ пользователя** в БД. Значения вида `web_*` валидны. На путях `api/create-payment.js`, `api/yookassa-webhook.js`, `api/cron-subscriptions.js`, `api/cancel-subscription.js` и во фронтовой отмене автосписания нельзя завязываться на `contact_id` и нельзя требовать «настоящий числовой Telegram ID».
...
- `POST /api/cancel-subscription` отменяет только автосписание (`payment_method_id = NULL`), но не деактивирует подписку мгновенно — она действует до `subscription_end`
...
- Амбассадорские комиссии: 30% (level 1) и 5% (level 2) — **намеренно отключены**; функция `processCommissions()` вызывалась только из webhook и удалена как мёртвый код 2026-07-24 (логику можно восстановить из git-истории при необходимости)
...
История подарочных периодов отображается в `/profile` через компонент `src/components/GiftPeriodsHistory.js`. Данные берёт из `GET /api/get-gift-windows` (поддерживает GET с `?telegram_id=` и POST с `{ telegram_id }`). Статусы окон: `available` (доступно), `claimed` (получено), `expired` (прошёл — подарок недоступен).

### Email Notifications (`api/_lib/email-notifications.js`)

Отправляет письма через Resend API (`RESEND_API_KEY`). Два события:
- `gift_available` — подарок доступен для получения (начало нового окна)
- `auto_debit_tomorrow` — завтра автосписание (предупреждение за 1 день)

Дедупликация через таблицу `email_notifications_log` — одно письмо на событие в день. Поле `users.email` — адрес для уведомлений, заполняется при OTP-входе. Крон вызывает уведомления из `api/cron-subscriptions.js`.
...
### Admin UI (AdminPage.js)

Вход: тройной клик по логотипу → пароль (env `ADMIN_PASSWORD`). Дизайн: cyberpunk (#07070f, неон cyan/pink, monospace, шрифт Montserrat).

**Навигация**: на мобильных — кнопка ☰ открывает bottom-sheet со всеми 10 табами. На десктопе — горизонтальное меню.

Табы: `products` / `subscribers` / `banners` / `pricing` / `stats` / `add` / `add-product` / `orders` / `referrals` / `upsell`

- `products` — карточки товаров в 2/3/4 колонки; при клике открывается bottom-sheet шторка с: вкл/выкл, ценой, полем `discount` (индивидуальная скидка %), роль товара (кнопки ☆/★ для upsell/promo/threshold gifts)
- `subscribers` («Люди») — адаптивная сетка карточек (1/2/3/4 колонки); клик → bottom-sheet детальной карточки с кнопками подарков (+/−, зелёные/красные), сменой тарифа, SHC, тегами, заметками; кнопки 290/490/1190 → inline datepicker → `/api/admin/set-subscription`
- `stats` — раздельная выручка Подписки (payments) vs Заказы (orders); SVG-график заказов по дням; периоды 1/7/15/30 дней
- `orders` — подарочные заказы с поиском
- `referrals` — топ рефереров + история SHC
- `upsell` — список SKU допродаж + promo/threshold-gifts
...
- `GET /PUT /api/admin/products` — список товаров каталога + обновление. PUT принимает `{ catalog, index, enabled, price, discount }`. Поле `discount` — индивидуальная скидка в % (0 = нет скидки), хранится в `data/products/` JSON рядом с `price` и `enabled`.
...
Маршрут `/game` → `WordlePage.js`. Доступна только подписчикам (проверка `subscriptionStatus === 'активно'`). `FloatingGameWidget` — компонент плавающего виджета, **отключён** в `App.js` (закомментирован). Кнопка «Игры» в шапке `/discount-shop` открывает `GamesModal` с правилами.
...
- `POST /api/game-guess` — принимает `{ word, attempt }` (только русские буквы, 5 символов, слово из словаря). Возвращает `{ result, isWon, winsToday?, shcEarned?, reveal? }`. При победе начисляет **5 SHC** (максимум 3 победы в день → макс 15 SHC/день). При 6-й неудачной попытке возвращает `reveal: targetWord`.
...
**Клавиатура**: iOS dark стиль — подложка `#1c1c1e`, клавиши `#6b6b6d`, синий Return `#0a84ff`. Раскладка ЙЦУКЕН 11/11/9 + Enter/⌫. Long-press на «Е» → popup «Ё», long-press на «Ь» → popup «Ь/Ъ». UX: popup появляется на touchstart/pointerdown, выбор — поднять палец над нужной буквой (tap-выбор, не drag).

**⚠️ Ловушка словаря**: seed БД при старте (`db.js`/`db-pg.js`) должен идти из `require('../game-words.json')` (480 curated слов), а НЕ из `loadFiveLetterWords()` (6074 слова из npm-пакетов). Эти 6074 слова — только для валидации гессов (in-memory `VALID_WORDS` Set).
...
RESEND_API_KEY                          # Email notifications via Resend (gift available, auto-debit tomorrow)
```
...
Vault — долговременная база знаний проекта. Он не является обязательным местом сохранения текущей сессии и не заменяет handoff между Codex и Claude Code.

Обязательное сохранение текущей сессии:
1. Использовать внешний протокол `C:\Users\User\.agents\skills\save-session\SKILL.md`.
2. Записать результат в [session-handoffs/current.md](session-handoffs/current.md) внутри этого проекта.
3. Только после этого, при необходимости, обновить Obsidian как дополнительную проектную память.
...
1. Сначала обнови [session-handoffs/current.md](session-handoffs/current.md) по протоколу `C:\Users\User\.agents\skills\save-session\SKILL.md`.
2. Если нужен долговременный лог проекта, дополнительно создай заметку в `obsidian-vault/sessions/` с именем `YYYY-MM-DD — краткое описание.md`.
3. Запиши в Obsidian-заметке: что сделано, какие файлы изменены, коммиты, решения, открытые вопросы.
4. Если обнаружен и исправлен баг — добавь заметку в `obsidian-vault/knowledge/debugging/`.
5. Если принято архитектурное решение — добавь в `obsidian-vault/knowledge/decisions/`.
6. При необходимости обнови `obsidian-vault/00-home/текущие приоритеты.md`:
...
2. Сначала обнови [session-handoffs/current.md](session-handoffs/current.md) по протоколу `C:\Users\User\.agents\skills\save-session\SKILL.md`.
3. Если нужен долговременный лог проекта, дополнительно создай заметку в `obsidian-vault/sessions/` с пометкой `[ОТКАТ]` в названии.
4. В Obsidian-заметке опиши: что пробовали, почему не сработало, что откатили.
5. Если баг — добавь в `obsidian-vault/knowledge/debugging/` с описанием ловушки.
6. В `текущие приоритеты.md` задачу НЕ отмечай выполненной — оставь в «Срочно» или «Важно».
---

## Паритет Codex и Claude Code

Этот проект подключён к общему workspace-мозгу:

- главный источник правды уровнем выше: [../AGENTS.md](../AGENTS.md);
- общий handoff проекта: [session-handoffs/current.md](session-handoffs/current.md);
- внешний протокол сохранения сессии: C:\Users\User\.agents\skills\save-session\SKILL.md;
- после значимых изменений применять правило workspace-brain-sync: локальное остаётся в проекте, повторяющееся уходит в ../ai-clone/, процедуры — в skills.
