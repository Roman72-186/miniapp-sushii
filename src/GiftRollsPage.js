...
function GiftRollsPage() {
  ...
  const fetchRolls = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/подписка 490/rolls-490.json?v=' + Date.now());
      if (!res.ok) throw new Error('Ошибка загрузки');
      const data = await res.json();
      setProducts(
        data.items.filter(item => item.enabled !== false).map((item, idx) => ({
          id: item.sku || `gift-roll-${idx}`,
          name: item.name,
          sku: item.sku,
          price: 0,
    ...
    <div className="shop-page">
      <header className="shop-header">
        <div className="shop-header__center">
          ...
          <span className="shop-header__title">Роллы в подарок</span>
        </div>
        <div className="shop-header__spacer" />
      </header>
      ...
      <div className="shop-section">
        <p style={{ color: '#999', fontSize: 13, margin: '8px 16px 0', padding: 0 }}>
          Выберите один ролл — входит в вашу подписку
        </p>
      </div>
