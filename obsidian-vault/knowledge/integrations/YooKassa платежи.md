...
Важно:
- `telegram_id` на этих путях используется как строковый ключ пользователя, а не обязательно как числовой Telegram chat id
- ID вида `web_*` валидны для создания платежа, webhook, cron-автосписания и отмены автосписания
- `contact_id` для отмены автосписания не нужен: backend его не использует
- отмена автосписания не деактивирует подписку мгновенно, а только выключает следующее списание
- Telegram-уведомления о продлении/ошибке списания могут не доходить до `web_*` пользователей, но сама платёжная логика при этом работает
